...
async function handleExtensionMessage(event) {
  ...
  switch (message.type) {
    ...
    case 'mindmapStarted':
      console.log('Mindmap generation started in extension');
      break;

    case 'mindmapCompleted':
      console.log('Mindmap generation completed');
      if (mindmapBtn) {
        mindmapBtn.disabled = false;
        mindmapBtn.style.opacity = '1';
        mindmapBtn.title = 'Generate Mermaid mindmap from this PDF';
      }
      break;

    case 'mindmapError':
      console.error('Mindmap generation error:', message.error);
      if (mindmapBtn) {
        mindmapBtn.disabled = false;
        mindmapBtn.style.opacity = '1';
        mindmapBtn.title = 'Generate Mermaid mindmap from this PDF';
...
export function requestMindmap() {
  console.log('Requesting document mindmap...');
  postMessage({
    type: 'mindmapRequest',
    fileName: PDF_CONFIG.fileName,
    isUrl: PDF_CONFIG.isUrl,
