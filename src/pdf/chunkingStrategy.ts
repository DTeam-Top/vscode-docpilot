...
function createOverlap(text: string, overlapRatio: number): string {
...
export function getDefaultConfig(maxInputTokens: number): ChunkingConfig {
...
export function createSemanticChunks(pdfText: string, config: ChunkingConfig): DocumentChunk[] {
  ...
  for (let i = 1; i < pages.length; i += 2) {
    ...
    const paragraphs = config.paragraphBoundary ? splitIntoParagraphs(pageContent) : [pageContent];
    ...
    for (const paragraph of paragraphs) {
      ...
      if (currentTokens + paragraphTokens > config.maxTokensPerChunk && currentChunk) {
        ...
        const overlapContent = createOverlap(currentChunk, config.overlapRatio);
...
export function validateChunks(chunks: DocumentChunk[], config: ChunkingConfig): boolean {
  for (const chunk of chunks) {
    if (chunk.tokens > config.maxTokensPerChunk * 1.1) {
      // Allow 10% tolerance
      logger.warn(`Chunk ${chunk.index} exceeds token limit`, {
